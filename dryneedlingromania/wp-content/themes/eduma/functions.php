...
const THIM_THEME_VERSION = '5.2.7';
	...
	foreach ( $theme_options as $key => $val ) {
		...
		if ( is_array( $val_opt ) ) {
			...
			if ( $val_opt != '' ) {
				...
				if ( $key == 'thim_body_primary_color' || $key == 'thim_button_hover_color' ||
					$key == 'thim_main_menu_text_color' || $key == 'thim_sticky_main_menu_text_color' ||
					$key == 'thim_mobile_menu_text_color' ) {
					if ( $val_opt[0] == '#' ) {
...
// Added by Vlad Bulete - start region
function custom_order_events_by_start_date($pieces, $args) {
    if( $args['orderby'] == 'event_start_date' ) {
        $pieces['orderby'] = 'ORDER BY event_start_date ASC';
    }
    return $pieces;
}
add_filter('em_events_get_sql', 'custom_order_events_by_start_date', 10, 2);

// end of Vlad Bulete region