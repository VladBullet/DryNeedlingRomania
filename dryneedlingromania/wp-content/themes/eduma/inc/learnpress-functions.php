...
if ( ! function_exists( 'thim_course_info' ) ) {
	function thim_course_info( $for_single = true ) {
		...
		if ( thim_is_new_learnpress( '4.0' ) ) {
			...
			$course_duration    = learn_press_get_post_translated_duration( get_the_ID() );
		} else {
			$course_skill_level   = get_post_meta( $course_id, 'thim_course_skill_level', true );
			$thim_course_duration = get_post_meta( $course_id, 'thim_course_duration', true );
			$course_duration      = thim_get_post_translated_duration( $thim_course_duration, esc_html__( 'Lifetime access', 'learnpress' ) );
		}
			...
			<ul>
				...
				<?php if ( ! empty( $course_duration ) ) : ?>
					<li class="duration-feature">
						...
						<span
							class="value"><?php echo $course_duration; ?></span>
					</li>
				<?php endif; ?>
